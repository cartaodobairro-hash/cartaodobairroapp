...
function PaymentStep() {
  ...
  const { data: pendingPayment } = useQuery({
    ...
    queryFn: async () => {
      ...
      const selectedPaymentId = search.order_nsu && search.order_nsu !== subscription.id ? search.order_nsu : checkoutPaymentId;
      if (selectedPaymentId) query = query.eq("id", selectedPaymentId);
