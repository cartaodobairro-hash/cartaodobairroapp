CREATE OR REPLACE FUNCTION public.activate_subscription_by_id(_subscription_id uuid, _amount numeric DEFAULT NULL::numeric, _transaction_id text DEFAULT NULL::text, _payment_id uuid DEFAULT NULL::uuid)
RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_sub public.subscriptions%ROWTYPE; v_period text; v_next date; v_payment_id uuid;
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.payments WHERE transaction_id = _transaction_id AND status = 'pago') THEN
    RETURN EXISTS (SELECT 1 FROM public.payments WHERE transaction_id = _transaction_id AND subscription_id = _subscription_id AND status = 'pago' AND (_payment_id IS NULL OR id = _payment_id));
  END IF;
  ...
  WHERE subscription_id = _subscription_id AND status = 'pendente' AND amount = _amount
    AND (_payment_id IS NULL OR id = _payment_id)
  ...
  v_next := CASE WHEN v_period = 'anual' THEN
    ((CASE WHEN v_sub.status = 'ativo' THEN GREATEST(CURRENT_DATE, v_sub.next_due_date) ELSE CURRENT_DATE END) + INTERVAL '1 year')::date
    ELSE ((CASE WHEN v_sub.status = 'ativo' THEN GREATEST(CURRENT_DATE, v_sub.next_due_date) ELSE CURRENT_DATE END) + INTERVAL '1 month')::date END;
