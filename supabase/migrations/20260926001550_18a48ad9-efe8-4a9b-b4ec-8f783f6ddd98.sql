...
BEGIN
  ...
  IF v_role = 'customer'::public.app_role THEN
    ...
    -- Create the customer at registration, not at plan selection or payment.
...
-- Recover customer registrations that predate immediate customer creation.
INSERT INTO public.customers (user_id, seller_id, plan_id, status)
SELECT u.id,
  CASE WHEN l.id IS NOT NULL THEN l.seller_id WHEN nullif(btrim(u.raw_user_meta_data->>'proposal_token'),'') IS NULL THEN s.id ELSE NULL END,
  l.plan_id,
  'pendente'::public.generic_status
FROM auth.users u
JOIN public.user_roles r ON r.user_id = u.id AND r.role = 'customer'
LEFT JOIN public.customers c ON c.user_id = u.id
LEFT JOIN public.seller_leads l ON l.proposal_token::text = u.raw_user_meta_data->>'proposal_token'
  AND l.status <> 'perdido' AND l.customer_id IS NULL
  AND lower(btrim(l.email)) = lower(btrim(u.email))
  AND regexp_replace(coalesce(l.cpf,''),'\D','','g') = regexp_replace(coalesce(u.raw_user_meta_data->>'cpf',''),'\D','','g')
  AND length(regexp_replace(coalesce(u.raw_user_meta_data->>'cpf',''),'\D','','g')) = 11
  AND EXISTS (SELECT 1 FROM public.sellers ps WHERE ps.id = l.seller_id AND ps.status = 'ativo'
    AND (nullif(btrim(u.raw_user_meta_data->>'seller_code'),'') IS NULL OR lower(ps.seller_code) = lower(u.raw_user_meta_data->>'seller_code')))
LEFT JOIN public.sellers s ON lower(s.seller_code) = lower(u.raw_user_meta_data->>'seller_code') AND s.status = 'ativo'
WHERE c.id IS NULL
ON CONFLICT (user_id) DO NOTHING;

UPDATE public.seller_leads l SET customer_id = c.id, status = 'cadastro', last_contact_at = now()
FROM auth.users u JOIN public.customers c ON c.user_id = u.id
WHERE l.proposal_token::text = u.raw_user_meta_data->>'proposal_token'
  AND l.seller_id = c.seller_id AND l.customer_id IS NULL AND l.status <> 'perdido'
  AND lower(btrim(l.email)) = lower(btrim(u.email))
  AND regexp_replace(coalesce(l.cpf,''),'\D','','g') = regexp_replace(coalesce(u.raw_user_meta_data->>'cpf',''),'\D','','g')
  AND length(regexp_replace(coalesce(u.raw_user_meta_data->>'cpf',''),'\D','','g')) = 11;